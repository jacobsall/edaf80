...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		std::stack<CelestialBodyRef> unprocessed;
		CelestialBodyRef sunRef;
		sunRef.body = &sun;
		sunRef.parent_transform = glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f));
		unprocessed.push(sunRef);
