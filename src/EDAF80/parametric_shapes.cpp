...
bonobo::mesh_data
parametric_shapes::createLine(float const length,  float const lineWidth) {
	auto const vertices = std::array<glm::vec3, 5>{
                              ...
                              unsigned int const vertical_split_count)
{
	...
	if (horizontal_split_count > 0u || vertical_split_count > 0u)
	{
		...
		for (unsigned int x = 0u; x < horizontal_split_count; x++)
		{
			...
			{
				tess_vertices[index] = glm::vec3((width / horizontal_split_count) * x, 0, (height / vertical_split_count) * z);

				texcoords[index] = glm::vec3(static_cast<float>(x) / (static_cast<float>(horizontal_split_count)),
		...
		for (unsigned int x = 0u; x < horizontal_split_count - 1; x++)
		{
			...
			{
				tess_index_sets[index] = glm::uvec3(x * vertical_split_count + z,
					x * vertical_split_count + z + 1u,
					x * vertical_split_count + z + vertical_split_count);
				index++;

				tess_index_sets[index] = glm::uvec3(x * vertical_split_count + z + vertical_split_count,
					x * vertical_split_count + z + 1u,
                                ...
                                unsigned int const latitude_split_count)
{
	...
	for (unsigned int i = 0u; i < longitude_vertices_count; ++i) {
		...
		for (unsigned int j = 0u; j < latitude_vertices_count; ++j) {
			...
			//auto const t = glm::vec3(radius / 2 * (std::sin(theta + phi) - std::sin(theta - phi)), 0.0f, -radius / 2 * (std::cos(theta - phi) - std::cos(theta + phi)));
			tangents[index] = t;
